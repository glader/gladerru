...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for movie in orm.Movie.objects.all():
            if movie.year:
                refresh = datetime.datetime(int(movie.year), 9, 1)
            else:
                refresh = datetime.datetime(2000, 9, 1)

            if not movie.dt_soundtrack_added:
                movie.dt_soundtrack_added = refresh
            if not movie.dt_teaser_added:
                movie.dt_teaser_added = refresh
            if not movie.dt_fullmovie_added:
                movie.dt_fullmovie_added = refresh
            movie.save()
