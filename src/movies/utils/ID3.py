...
class ID3:
    ...
    def tupleize(self, s):
        if self.as_tuple and not isinstance(s, types.TupleType):
            return (s,)
        else:
            return s
    ...
    def legal_genre(self, genre):
        if isinstance(genre, types.IntType) and 0 <= genre < len(self.genres):
            return 1
        else:
            return 0
    ...
    def __setitem__(self, k, v):
        key = k
        if key not in ['TITLE', 'ARTIST', 'ALBUM', 'YEAR', 'COMMENT', 'TRACKNUMBER', 'GENRE']:
            return
        if k == 'TRACKNUMBER':
