...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for post in orm.Item.objects.filter(type=1):
            ...
            orm.Comment.objects.filter(content_type=1, object_id=post.pk).update(content_type=52, object_id=new_post.pk)
            orm.ItemVote.objects.filter(content_type=1, object_id=post.pk).update(content_type=52, object_id=new_post.pk)

        db.execute("UPDATE core_post SET author_id=1 WHERE author_id is NULL")
