...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        from core.templatetags.content import parser

        for post in orm.Word.objects.all():
            post.abstract = parser(post.abstract)
            post.content = parser(post.content)
            post.save()

        for post in orm.Mountain.objects.all():
            post.content = parser(post.content)
            post.save()

        for post in orm.Movie.objects.all():
            post.content = parser(post.content)
            post.save()

        for post in orm.Man.objects.all():
            post.content = parser(post.content)
            post.save()

        for post in orm.Studio.objects.all():
            post.content = parser(post.content)
            post.save()

        for post in orm.Rubric.objects.all():
            post.content = parser(post.content)
            post.abstract = parser(post.abstract)
            post.save()
    ...
    models = {
        ...
        u'core.usernews': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'news': ('django.db.models.fields.related.ForeignKey', [], {'default': 'None', 'to': u"orm['core.News']", 'null': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['auth.User']"})
        },
        u'core.uservisitstat': {
